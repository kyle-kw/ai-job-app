...
```powershell
$previousVersion = '<previous-major.minor.patch>'
...
- 将 `$previousVersion` 作为基线版本只通过手动安装分发，并在说明中注明未进行平台代码签名/公证。
- 用真实 Windows 10 x64、macOS 12 Intel、macOS 12 Apple Silicon 安装 `v$previousVersion`。
- 发布 `v$version` 后，分别验证提示、下载进度、签名校验、安装和重启。
- 三个平台全部升级成功后，才将 `v$version` Release 从 Draft 公开。
