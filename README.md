...
本地优先的 AI 求职桌面助手：抓取并去重 BOSS 岗位、用 LLM 批量提取岗位/公司/工商信息、生成全量岗位数据报告、导入和优化简历、评估岗位匹配度、审核专岗简历改写，并生成招呼语。
...
- BOSS 抓取只由用户主动触发，使用独立 Chrome Profile，不绕过验证码；抓取完成后自动关闭该专用 Chrome。
