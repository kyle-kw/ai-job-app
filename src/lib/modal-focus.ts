...
function isVisible(element: HTMLElement, root: HTMLElement) {
  ...
  return typeof checkVisibility !== 'function' || checkVisibility.call(element, { checkOpacity: false, checkVisibilityCSS: true });
}
...
function suppressBackground(node: HTMLElement) {
  ...
  while (current.parentElement) {
    ...
    for (const sibling of Array.from(parent.children)) {
      ...
      const clickableBackdrop = sibling instanceof HTMLButtonElement
...
export function modalFocus(node: HTMLElement, options: ModalFocusOptions) {
  ...
  const focusable = () => [...node.querySelectorAll<HTMLElement>(FOCUSABLE)].filter((element) => isVisible(element, node));
  const focusInitial = () => {
    if (!isTopmost()) return;
    const preferred = current.initialFocus ? node.querySelector<HTMLElement>(current.initialFocus) : null;
    ((preferred && isVisible(preferred, node) ? preferred : null) ?? focusable()[0] ?? node).focus();
  };
