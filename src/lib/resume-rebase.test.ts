...
describe('resume variant rebase', () => {
  it('automatically takes untouched master fields and asks for true conflicts', () => {
    ...
    const master = { ...structuredClone(base), headline: '高级 AI 工程师', summary: '主简历新简介', version: base.version + 1 };
    const variant = { ...structuredClone(base), id: 'variant', summary: '岗位定制简介', version: 2 };
    ...
    const applied = applyResumeRebase(variant, master, preview, [{ path: '/summary', choice: 'variant' }]);
