import type { ResumeProfile, ResumeRebaseChange, ResumeRebasePreview, ResumeRebaseResolution } from '$lib/types';

const fields: ReadonlyArray<[keyof ResumeProfile, string]> = [
  ['name', '姓名'], ['headline', '职业标题'], ['email', '邮箱'], ['phone', '电话'], ['location', '所在地'],
  ['website', '个人主页'], ['summary', '个人简介'], ['templateId', '简历结构模板'],
  ['professionalSkills', '专业技能'], ['experiences', '工作经历'], ['education', '教育经历'],
  ['projects', '项目经历'], ['certifications', '证书 / 专业资质']
];
...
export function buildResumeRebasePreview(
  ...
  for (const [key, label] of fields) {
    const change: ResumeRebaseChange = {
      path: `/${key}`, label,
      base: structuredClone(base[key]), master: structuredClone(master[key]), variant: structuredClone(variant[key])
    };
    if (equal(variant[key], base[key]) && !equal(master[key], base[key])) autoChanges.push(change);
    else if (!equal(variant[key], base[key]) && !equal(master[key], base[key]) && !equal(variant[key], master[key])) conflicts.push(change);
  }
  return { variantId, variantVersion, baseResumeVersion, masterVersion: master.version, autoChanges, conflicts };
}
...
export function applyResumeRebase(
  ...
  for (const change of preview.autoChanges) next[change.path.slice(1)] = structuredClone(change.master);
