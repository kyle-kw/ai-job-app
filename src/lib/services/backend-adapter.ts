...
export function createBackendAdapter(useBrowser: boolean) {
  const adapter = {
    async bootstrap(): Promise<BootstrapSnapshot> {
      if (useBrowser) return structuredClone(mockState);
    ...
    async listReportKeywords(): Promise<ReportKeyword[]> {
      if (useBrowser) return structuredClone(currentMockReportKeywords());
    ...
    async getReportCompetitivenessState(
      keywordKeys: string[]
      ...
      if (useBrowser) {
        ...
        advanceMockTask(
          ...
          () => {
            ...
            if (knownKeyword)
              mockKeywordJobs[knownKeyword.key] = mockJobsState.map((job) => job.id);
    ...
    async applyResumeVariantRebase(
      ...
      if (useBrowser) {
        ...
        const committed = await this.saveResumeVariant(
          variantId,
          candidate,
    ...
    async proposeResumeChatEdits(request: ResumeChatRequest): Promise<ResumeChatProposal> {
      if (useBrowser) {
        ...
        if (request.marketContext) {
          const keywordKeys = [
            ...new Set(
              request.marketContext.keywordKeys
    ...
    async downloadAndInstallUpdate(onEvent: (event: UpdateEvent) => void): Promise<void> {
      if (useBrowser) throw new Error('更新安装仅桌面版可用');
    ...
    async createBackup(outputPath: string): Promise<BackupInfo> {
      if (useBrowser) throw new Error('备份导出仅桌面版可用');
      return invoke('create_backup', { outputPath });
    },

    async restoreBackup(backupPath: string): Promise<void> {
      if (useBrowser) throw new Error('备份恢复仅桌面版可用');
    ...
    async exportDiagnostics(outputPath: string): Promise<string> {
      if (useBrowser) throw new Error('诊断导出仅桌面版可用');
