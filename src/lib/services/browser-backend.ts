...
export const browserBackend = {
  ...
  async listReportKeywords(): Promise<ReportKeyword[]> {
    return structuredClone(currentMockReportKeywords());
  },
  ...
  async exportJobsJson(outputPath: string, _query?: JobQuery): Promise<RenderResult> {
    return {
      path: outputPath || 'browser-demo://岗位数据.json',
      fileName: outputPath.split(/[\\/]/).at(-1) || '岗位数据_demo.json'
    };
  },
  async exportJobDataReport(_keywordKeys: string[], outputPath: string): Promise<RenderResult> {
    return {
  ...
  async generateReportCompetitiveness(
    ...
    const analysis: ReportCompetitivenessAnalysis = {
      ...
      items: local.items.map((item) => ({
        ...item,
        rationale: `AI 语义复核：${item.rationale}`
      }))
    };
    ...
    return this.getReportCompetitivenessState(keywordKeys);
  },
  async getInterviewPreparationState(_keywordKeys: string[]): Promise<InterviewPreparationState> {
  ...
  async previewResumeVariantRebase(variantId: string): Promise<ResumeRebasePreview> {
    ...
    const base = mockVersions.find(
      (item) => item.resumeId === variant.baseResumeId && item.version === variant.baseResumeVersion
    )?.profile;
  ...
  async applyResumeVariantRebase(
    ...
    const preview = await this.previewResumeVariantRebase(variantId);
    const candidate = applyResumeRebase(variant.profile, master, preview, resolutions);
    const committed = await this.saveResumeVariant(variantId, candidate, expectedVariantVersion);
  ...
  async restoreResumeVariantVersion(
    ...
    const result = await this.saveResumeVariant(variantId, version.profile, expectedVersion);
  ...
  async startFitBatchForQuery(query: JobQuery): Promise<string> {
    return this.startFitBatch(filterJobs(mockJobsState, query).map((job) => job.id));
  },
  ...
  async applyResumeChatEdits(request: ApplyResumeEditsRequest): Promise<ResumeEditCommitResult> {
    if (request.proposal.target.kind === 'variant') {
      ...
      const result = await this.saveResumeVariant(variant.id, next, request.expectedVersion);
  ...
  async checkForUpdate(_manual = true): Promise<AppUpdateInfo | null> {
    return null;
  },
  async downloadAndInstallUpdate(_onEvent: (event: UpdateEvent) => void): Promise<void> {
    throw new Error('更新安装仅桌面版可用');
  },
  async createBackup(_outputPath: string): Promise<BackupInfo> {
    throw new Error('备份导出仅桌面版可用');
  },
  async restoreBackup(_backupPath: string): Promise<void> {
    throw new Error('备份恢复仅桌面版可用');
  },
  ...
  async exportDiagnostics(_outputPath: string): Promise<string> {
    throw new Error('诊断导出仅桌面版可用');
  },
