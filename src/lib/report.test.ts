...
import {
  buildClientJobDataReport,
  buildScrapeSampleSummary,
  classifyJobRole,
  parseMonthlySalary
} from '$lib/report';
...
describe('job data report', () => {
  ...
  it('compares the latest two identical cross-day scrape batches', () => {
    ...
    expect(
      report.batchComparison.skillChanges.find((item) => item.label === 'RAG')
  ...
  it('reports sample limitations and refuses non-comparable ranges', () => {
    ...
    expect(report.batchComparison).toMatchObject({
      status: 'unavailable',
      reason: 'multi_keyword'
    });
    expect(report.sampleQuality.detail.coverage).toBe(0);
    expect(report.sampleQuality.limitations).toEqual(
      expect.arrayContaining([
        expect.stringContaining('有限页 BOSS'),
        expect.stringContaining('少于 20'),
        expect.stringContaining('薪资覆盖不足')
      ])
    );
  });
});
