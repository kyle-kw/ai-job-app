...
export function resumeSearchFields(resume: ResumeProfile): Array<{ path: string; text: string }> {
  return [
    { path: '/headline', text: resume.headline },
    { path: '/summary', text: resume.summary },
    ...resume.professionalSkills.map((group, index) => ({
      path: `/professionalSkills/${index}`,
      text: `${group.label} ${group.items.join(' ')}`
    })),
    ...resume.experiences.map((item, index) => ({
      path: `/experiences/${index}`,
      text: `${item.company} ${item.position} ${item.highlights.join(' ')}`
    })),
    ...
    ...resume.education.map((item, index) => ({
      ...
      text: `${item.institution} ${item.area} ${item.degree} ${item.degreeDetail} ${item.highlights.join(' ')}`
    })),
...
export function coverageRequirements(
  job: Job
): Array<{ id: string; label: string; kind: ResumeCoverageItem['kind'] }> {
  const values: Array<{ label: string; kind: ResumeCoverageItem['kind'] }> = [];
  for (const requirement of job.structuredDetails?.requirements ?? [])
    values.push({ label: requirement, kind: 'requirement' });
...
export function summarizeCoverage(
  report: Omit<
    ResumeCoverageReport,
    'coveredCount' | 'strengthenableCount' | 'gapCount' | 'unknownCount'
  >
): ResumeCoverageReport {
  return {
    ...report,
...
export function coverageHighlightKeywords(report: ResumeCoverageReport | null): string[] {
  ...
  return report.items
    .filter(
      (item) =>
        item.kind === 'skill' && (item.status === 'covered' || item.status === 'strengthenable')
    )
    .map((item) => item.label);
}
