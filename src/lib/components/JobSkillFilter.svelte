...
<div class="relative" use:outside>
  ...
  >
    <span class={selected.length ? 'text-ink' : 'body-muted'}
      >{selected.length ? `已选 ${selected.length} 项` : '不限'}</span
    >
  ...
  {#if open}
    <div
      class="absolute left-0 right-0 top-full z-30 mt-1 rounded-xl border bg-panel p-2 shadow-xl"
    ...
    >
      ...
      <div
        id="job-skill-options"
        class="scrollbar-thin mt-2 max-h-52 overflow-y-auto"
        role="listbox"
        aria-label="技能选项"
            ...
            >
              <span class="flex min-w-0 items-center gap-2"
                ><span
                  ...
                  style="border-color: var(--line);"
                  >{#if selectedKeys.has(option.label.toLocaleLowerCase())}<Check
                      size={11}
                      class="text-brand"
                    />{/if}</span
                ><span class="truncate">{option.label}</span></span
              >
  ...
  {#if selected.length}
    <div class="mt-2 flex flex-wrap gap-1.5" aria-label="已选技能">
      {#each selected as skill}
        <button
          type="button"
          class="chip gap-1 px-2 py-1"
          aria-label={`移除技能 ${skill}`}
          on:click={() => toggleSkill(skill)}>{skill}<X size={11} /></button
        >
      {/each}
    </div>
  {/if}
