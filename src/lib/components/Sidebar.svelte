...
<aside class="sidebar flex w-[224px] shrink-0 flex-col border-r px-3 py-4" style="background: var(--sidebar); border-color: var(--line);">
  ...
  <div class="mt-auto">
    <div class="local-note mx-2 mb-3 rounded-xl border p-3" style="border-color: var(--line); background: var(--brand-faint);">
...
<style>
  ...
  .nav-item:hover { background: color-mix(in srgb, var(--panel) 62%, transparent); color: var(--ink); }
  .nav-item.active { background: var(--panel); color: var(--brand); box-shadow: 0 1px 2px rgba(15,23,42,.04); }
  @media (max-width: 1180px) {
    .sidebar { width: 80px; padding-left: 10px; padding-right: 10px; }
    .brand-copy, .nav-copy, .local-note { display: none; }
    .sidebar > a { justify-content: center; padding-left: 0; padding-right: 0; }
