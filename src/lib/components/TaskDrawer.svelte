    ...
    task.state === 'completed'
      ...
      : task.state === 'failed'
        ...
        : task.state === 'running'
          ? CircleDashed
          : Clock3;
  const labelFor = (task: TaskRun) =>
    ({
      scrape: '岗位抓取',
      'job-detail-extraction': '岗位详情提取',
      'resume-import': '简历解析',
      fit: '匹配分析',
      tailor: '专岗优化',
      render: 'PDF 渲染',
...
{#if open}
  ...
  ></button>
  <div
    class="fixed bottom-0 right-0 top-0 z-50 flex w-[430px] flex-col border-l bg-panel shadow-2xl"
    style="border-color: var(--line); animation: slide-in .22s ease-out;"
    role="dialog"
    aria-modal="true"
    aria-labelledby="task-drawer-title"
    tabindex="-1"
    use:modalFocus={{ close: () => (open = false) }}
  >
    <div
      class="flex h-[74px] items-center justify-between border-b px-5"
    ...
    >
      ...
      <button class="btn-icon" on:click={() => (open = false)} aria-label="关闭"
        ><X size={18} /></button
      >
    </div>
    ...
    <div class="scrollbar-thin flex-1 overflow-y-auto px-5 py-5">
      {#if $runningTasks.length === 0 && $completedTasks.length === 0}
        <div class="grid min-h-72 place-items-center text-center">
          <div>
            <span class="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl surface-soft"
              ><Clock3 size={21} class="body-muted" /></span
            >
                ...
                <div class="flex items-start gap-3">
                  <span class="mt-0.5 text-brand"
                    ><svelte:component
                      this={iconFor(task)}
                      size={18}
                      class={task.state === 'running' ? 'animate-spin' : ''}
                    /></span
                  >
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center justify-between gap-3">
                      <p class="truncate text-sm font-semibold">{task.title}</p>
                      <span class="text-xs font-semibold text-brand">{task.progress}%</span>
                    </div>
                    <p class="mt-1 text-xs body-muted">{task.message}</p>
                    {#if task.kind === 'scrape'}<p
                        class="mt-2 text-[11px] font-medium text-warning"
                      >
                        请勿关闭应用，切换页面不会中断抓取。
                      </p>{/if}
                    <div class="mt-3 h-1.5 overflow-hidden rounded-full surface-soft">
                      <div
                        class="h-full rounded-full transition-all duration-500"
                        style={`width:${task.progress}%; background: var(--brand);`}
                      ></div>
                    </div>
                  </div>
                </div>
          ...
          <div class="space-y-2">
            {#each $completedTasks.slice(0, 8) as task}
              <article class="overflow-hidden rounded-xl border" style="border-color: var(--line);">
                <button
                  class="flex w-full items-center gap-3 px-3.5 py-3 text-left hover:surface-soft"
                  on:click={() => (expanded = expanded === task.id ? null : task.id)}
                >
                  <span
                    style={`color:${task.state === 'failed' ? 'var(--danger)' : 'var(--success)'}`}
                    ><svelte:component this={iconFor(task)} size={17} /></span
                  >
                  <div class="min-w-0 flex-1">
                    <p class="truncate text-sm font-medium">{task.title}</p>
                    <p class="mt-0.5 text-[11px] body-muted">{labelFor(task)} · {task.message}</p>
                  </div>
                  {#if expanded === task.id}<ChevronDown
                      size={15}
                      class="body-muted"
                    />{:else}<ChevronRight size={15} class="body-muted" />{/if}
                </button>
                {#if expanded === task.id}
