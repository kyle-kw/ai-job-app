<script lang="ts">
  ...
  $: selectedCityValid = citySelectionConfirmed
  ...
  const handleCityKeydown = (event: KeyboardEvent) => {
    ...
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
      ...
      activeCityIndex = activeCityIndex < 0
        ? (direction > 0 ? 0 : cityOptions.length - 1)
  ...
  <div class="fixed left-1/2 top-1/2 z-50 w-[640px] max-w-[calc(100vw-32px)] -translate-x-1/2 -translate-y-1/2 panel p-6" role="dialog" aria-modal="true" aria-labelledby="job-search-title" tabindex="-1" use:modalFocus={{ close, canClose: !scraping, initialFocus: '[aria-required="true"]' }}>
    ...
    {:else}
      <div class="grid grid-cols-2 gap-4">
        ...
        <label><span class="label">城市</span><div class="relative"><input class="input" role="combobox" aria-autocomplete="list" aria-expanded={cityMenuOpen} aria-controls="boss-city-options" aria-activedescendant={cityMenuOpen && activeCityIndex >= 0 ? `boss-city-option-${activeCityIndex}` : undefined} aria-invalid={!selectedCityValid} aria-describedby="boss-city-message" value={cityQuery} on:focus={openCityMenu} on:input={updateCityQuery} on:keydown={handleCityKeydown} on:blur={() => { cityMenuOpen = false; activeCityIndex = -1; }} />{#if cityMenuOpen}<div id="boss-city-options" class="absolute left-0 right-0 top-full z-20 mt-1 max-h-52 overflow-y-auto rounded-xl border bg-panel p-1 shadow-xl" style="border-color: var(--line);" role="listbox" aria-label="城市选项">{#if cityOptions.length}{#each cityOptions as city, index}<button id={`boss-city-option-${index}`} type="button" role="option" aria-selected={index === activeCityIndex} class="block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-brand-soft" class:bg-brand-soft={index === activeCityIndex} tabindex="-1" on:mousedown|preventDefault={() => selectCity(city)} on:click={() => selectCity(city)}>{city}</button>{/each}{:else}<p class="px-3 py-2 text-xs body-muted">未找到匹配城市</p>{/if}</div>{/if}</div><span id="boss-city-message" class="mt-1 block min-h-[16px] text-[11px] text-warning">{cityMessage}</span></label>
      ...
      <div class="mt-5 flex items-center justify-between gap-4"><p class="flex items-center gap-2 text-xs body-muted"><Info size={14} />下一步会说明登录检查方式，确认后才会启动。</p><button class="btn-primary shrink-0" disabled={scrapeTaskRunning || keywordMissing || !selectedCityValid} on:click={review}>{#if scrapeTaskRunning}<RefreshCw size={15} class="animate-spin" />已有任务运行{:else}<Search size={15} />开始抓取{/if}</button></div>
    {/if}
  </div>
{/if}
