...
describe('ResumeFactsEditor', () => {
  it('shows data-analysis guidance and adds a manual fact as pending by default', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('事实内容'), {
      target: { value: '使用 SQL 建立经营指标体系' }
    });
  ...
  it('resets confirmation after a semantic edit and keeps the resume body untouched', async () => {
    ...
    await fireEvent.input(screen.getByLabelText('事实内容'), {
      target: { value: 'RAG 检索命中率提升 25%' }
    });
    expect(confirmation).not.toBeChecked();
    await fireEvent.click(screen.getByRole('button', { name: '保存修改' }));

    const edited = changes.mock.calls[0][0].detail.facts.find(
      (fact: { id: string }) => fact.id === 'fact-rag'
    );
    expect(edited).toMatchObject({
      value: 'RAG 检索命中率提升 25%',
      source: '用户手工维护',
      confirmed: false
    });
  ...
  it('supplements finance facts without duplicating or confirming them', async () => {
    ...
    expect(
      facts.filter(
        (fact: { category: string; value: string }) =>
          fact.category === 'skill' && fact.value.toLowerCase() === 'excel'
      )
    ).toHaveLength(1);
  ...
  it('deletes only the selected fact after confirmation', async () => {
    ...
    await fireEvent.click(
      screen.getByRole('button', { name: `删除事实：${resume.facts[0].value}` })
    );
    expect(changes.mock.calls[0][0].detail.facts).toHaveLength(resume.facts.length - 1);
    expect(
      changes.mock.calls[0][0].detail.facts.some(
        (fact: { id: string }) => fact.id === resume.facts[0].id
      )
    ).toBe(false);
  });
});
