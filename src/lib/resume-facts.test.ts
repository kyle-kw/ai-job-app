...
describe('resume facts', () => {
  it('extracts role-neutral facts from every structured resume section', () => {
    ...
    resume.certifications = [
      { id: 'cert', name: '初级会计资格', issuer: '示例机构', date: '2024.01' }
    ];
    const facts = factsFromResumeContent(resume, () => `fact-${++id}`);

    expect(facts.some((fact) => fact.category === 'skill' && fact.value === 'SQL')).toBe(true);
    expect(
      facts.some((fact) => fact.category === 'experience' && fact.value.includes('云帆科技'))
    ).toBe(true);
    expect(facts.some((fact) => fact.category === 'project' && fact.value === '留存提升 8%')).toBe(
      true
    );
    expect(
      facts.some((fact) => fact.category === 'education' && fact.value.includes('浙江工业大学'))
    ).toBe(true);
    expect(
      facts.some((fact) => fact.category === 'certification' && fact.value.includes('初级会计资格'))
    ).toBe(true);
