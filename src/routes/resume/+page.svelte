<script lang="ts">
  ...
  const exportColorThemes: ReadonlyArray<{
    id: ResumeColorTheme;
    label: string;
    description: string;
    accent: string;
  ...
  $: draftPreferences = draft
    ...
    : null;
  $: effectiveDraft =
    draft && draftPreferences ? { ...draft, preferences: draftPreferences } : null;
  $: hasUnsavedChanges = Boolean(
    effectiveDraft &&
    baselineResume &&
    JSON.stringify(effectiveDraft) !== JSON.stringify(baselineResume)
  );
  $: if ($snapshot.resume && resumeMode === 'master') {
    ...
    const isNewVersion = Boolean(
      baselineResume &&
      incoming.id === baselineResume.id &&
      incoming.version > baselineResume.version
    );
  ...
  $: aiReady = Boolean(
    $snapshot.readiness.ai &&
    $snapshot.providers.some((provider) => provider.isDefault && provider.verified)
  );
  ...
  $: if (
    resumeMode === 'variant' &&
    activeVariant?.jobId &&
    activeVariant.jobId !== requestedVariantJobId
  ) {
    requestedVariantJobId = activeVariant.jobId;
    void loadActiveVariantJob(activeVariant.jobId);
  }
  $: localCoverageReport =
    activeVariantJob && effectiveDraft && activeVariant
      ? buildLocalResumeCoverage(
          activeVariantJob,
          { kind: 'variant', id: activeVariant.id },
          effectiveDraft
        )
      : null;
  $: coverageReport =
    ...
    !hasUnsavedChanges &&
    activeVariant &&
    aiCoverageReport.target.id === activeVariant.id &&
    aiCoverageReport.targetVersion === activeVariant.version
      ? aiCoverageReport
      : localCoverageReport;
  $: previewCoverageKeywords =
    resumeMode === 'variant' ? coverageHighlightKeywords(coverageReport) : [];
  ...
  function syncPreferenceTexts(preferences: JobPreferences) {
    targetRolesText = preferences.targetRoles.join('、');
    citiesText = preferences.cities.join('、');
    energizingText = preferences.energizingTasks.join('、');
    drainingText = preferences.drainingTasks.join('、');
    constraintsText = preferences.hardConstraints.join('、');
  }
  ...
  async function loadVariantJobs() {
    try {
      jobOptions = await backend.listJobOptions(jobQuery);
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async function loadActiveVariantJob(jobId: string) {
    ...
    try {
      activeVariantJob = await backend.getJob(jobId);
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async function switchResumeMode(mode: 'master' | 'variant') {
    ...
    if (hasUnsavedChanges) {
      const shouldSave = window.confirm(
        `当前${activeTargetLabel}有未保存修改。点击“确定”先保存，点击“取消”可选择放弃。`
      );
      if (shouldSave) {
  ...
  async function selectVariant(id: string) {
    ...
    try {
      adoptVariant(await backend.getResumeVariant(id));
      activeSection = 'content';
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error));
    }
  ...
  async function startVariantCreation() {
    if (hasUnsavedChanges && !window.confirm('放弃当前岗位版本的未保存修改并创建其他岗位版本？'))
      return;
    activeVariant = null;
    draft = null;
    baselineResume = null;
  ...
  async function createVariant() {
    ...
    try {
      const created = await backend.createResumeVariant(
        selectedVariantJobId,
        $snapshot.resume.version
      );
  ...
  async function save(): Promise<boolean> {
    ...
    try {
      if (resumeMode === 'variant') {
        ...
        const result = await backend.saveResumeVariant(
          activeVariant.id,
          structuredClone(effectiveDraft),
  ...
  async function openRebase() {
    ...
    if (!$snapshot.resume) {
      showToast('主简历暂时不可用，当前岗位版本仍可查看和导出，但不能同步。');
      return;
    }
    ...
    try {
      rebasePreview = await backend.previewResumeVariantRebase(activeVariant.id);
    } catch (error) {
      showToast(error instanceof Error ? error.message : String(error));
    } finally {
      rebaseLoading = false;
  ...
  async function applyRebase(resolutions: ResumeRebaseResolution[]) {
    ...
    try {
      const result = await backend.applyResumeVariantRebase(
        activeVariant.id,
        rebasePreview.variantVersion,
        rebasePreview.masterVersion,
        resolutions
      );
      adoptVariant(result.variant);
      variants = await backend.listResumeVariants();
  ...
  async function deleteActiveVariant() {
    if (
      !activeVariant ||
      !window.confirm(`删除岗位版本“${activeVariant.name}”及其历史记录？此操作不可恢复。`)
    )
      return;
    try {
  ...
  async function analyzeCoverage() {
    ...
    try {
      aiCoverageReport = await backend.analyzeResumeCoverage({
        kind: 'variant',
        id: activeVariant.id
      });
  ...
  async function savePrefs() {
    ...
    try {
      ...
      if (baselineResume)
        baselineResume = { ...baselineResume, preferences: structuredClone(preferences) };
  ...
  async function requestImport() {
    ...
    if (hasUnsavedChanges) {
      const shouldSave = window.confirm(
        '检测到未保存的简历修改。点击“确定”先保存；点击“取消”可选择放弃修改或终止导入。'
      );
      if (shouldSave) {
        if (!(await save())) return;
      } else {
        const shouldDiscard = window.confirm(
          '放弃当前未保存修改并重新导入？点击“取消”返回继续编辑。'
        );
  ...
  async function confirmPdfExport() {
    ...
    if (
      ...
      !window.confirm(`简历体检仍有 ${healthReport.errorCount} 个严重问题，仍要继续导出吗？`)
    )
      return;
    rendering = true;
    ...
    try {
      const fileName = safeResumeFileName(
        resumeMode === 'variant' && activeVariant
  ...
  async function openAssistant(
    ...
    initialPrompt = '',
    marketContext: MarketResumeContextRequest | null = null
  ) {
    if (saving) return;
    if (resumeMode === 'variant' && !$snapshot.resume) {
      showToast('主简历暂时不可用，当前岗位版本不能调用 AI。');
      return;
    }
    if (draft && hasUnsavedChanges) {
      const shouldSave = window.confirm(
        '检测到未保存的简历修改。点击“确定”先保存；点击“取消”可选择放弃修改或继续编辑。'
      );
      if (shouldSave) {
        if (!(await save())) return;
      } else {
        const shouldDiscard = window.confirm(
          '放弃当前未保存修改并打开 AI 对话？点击“取消”返回继续编辑。'
        );
        if (!shouldDiscard) return;
        if ($snapshot.resume) adoptResume($snapshot.resume);
      }
  ...
  async function handleResumeCommit(
    event: CustomEvent<ResumeCommitResult | ResumeVariantCommitResult>,
    ...
    if ('variant' in event.detail) {
      ...
      showToast(
        action === 'applied'
          ? `AI 修改已应用，已创建岗位版本 ${event.detail.variant.version}`
          : `已恢复并创建岗位版本 ${event.detail.variant.version}`
      );
    } else {
      ...
      showToast(
        action === 'applied'
          ? `AI 修改已应用，已创建版本 ${event.detail.resume.version}`
          : `已恢复并创建版本 ${event.detail.resume.version}`
      );
    }
  }
  ...
  function addSkillGroup() {
    if (!draft) return;
    draft.professionalSkills = [
      ...draft.professionalSkills,
      { id: crypto.randomUUID(), label: '新分组', items: [] }
    ];
  }

  function addSkill(groupIndex: number) {
    if (!draft) return;
    draft.professionalSkills = draft.professionalSkills.map((group, index) =>
  ...
  function applySuggestedGroups() {
    ...
    const additions = suggestedProfessionalSkillGroups(draft.templateId).filter(
      (group) => !existing.has(group.label)
    );
  ...
  function addCertification() {
    ...
    draft.certifications = [
      ...draft.certifications,
      { id: crypto.randomUUID(), name: '新证书', issuer: '', date: '' }
    ];
  }
  ...
  function updateHighlights(
    ...
    if (section === 'experiences') {
      draft.experiences = draft.experiences.map((item, index) =>
        index === itemIndex ? { ...item, highlights: update(item.highlights) } : item
      );
      ...
      draft.education = draft.education.map((item, index) =>
        index === itemIndex ? { ...item, highlights: update(item.highlights) } : item
      );
    }
  }

  function moveResumeItem(
    section: 'professionalSkills' | 'experiences' | 'projects' | 'certifications' | 'education',
    from: number,
    to: number
  ) {
    if (!draft) return;
    if (section === 'professionalSkills')
      draft.professionalSkills = moveItem(draft.professionalSkills, from, to);
      ...
      draft.certifications = moveItem(draft.certifications, from, to);
  ...
  function dropItem(event: DragEvent, kind: string, index: number, parent?: number) {
    ...
    if (kind.startsWith('highlight:') && parent !== undefined) {
      ...
      moveResumeItem(
        kind as 'professionalSkills' | 'experiences' | 'projects' | 'certifications' | 'education',
        dragSource.index,
        index
      );
    }
  ...
  async function focusHealthIssue(issue: ResumeHealthIssue) {
    ...
    for (const path of candidates) {
      const target = document.querySelector<HTMLElement>(
        `[data-resume-path="${CSS.escape(path)}"]`
      );
      if (!target) continue;
  ...
  function removeSkillGroup(groupIndex: number) {
    ...
    requestItemRemoval(`技能分组“${group.label || groupIndex + 1}”`, group, () => {
      if (draft)
        draft.professionalSkills = draft.professionalSkills.filter((item) => item.id !== group.id);
    });
  }
  ...
  function removeSkill(groupIndex: number, skillIndex: number) {
    ...
    requestItemRemoval(`技能“${skill || skillIndex + 1}”`, skill, () => {
      if (!draft) return;
      draft.professionalSkills = draft.professionalSkills.map((item) =>
        item.id === group.id ? { ...item, items: removeAt(item.items, skillIndex) } : item
      );
    });
  }
  ...
  function removeCertification(certificationIndex: number) {
    ...
    requestItemRemoval(
      `证书“${certification.name || certificationIndex + 1}”`,
      certification,
      () => {
        if (draft)
  ...
  function removeEducation(educationIndex: number) {
    ...
    if (!education) return;
    requestItemRemoval(
      `教育经历“${education.institution || educationIndex + 1}”`,
      education,
      () => {
  ...
  onMount(() => {
    ...
    if ($page.url?.searchParams.get('assistant') === '1') {
      ...
      const marketPrompt = marketMode
        ...
        : '';
      void openAssistant(
        marketMode ? null : $page.url.searchParams.get('job'),
        marketPrompt,
        marketContext
      );
    }
...
<div
  class="page-content flex h-[calc(100vh-74px)] min-h-[646px] max-w-none flex-col overflow-hidden pb-6"
>
  <div class="mb-5 flex shrink-0 items-end justify-between">
    <div class="min-w-0">
      <div class="mb-2 flex flex-wrap items-center gap-3">
        <div class="inline-flex rounded-xl p-1" style="background: var(--panel-soft);">
          <button
            class:active-mode={resumeMode === 'master'}
            class="mode-button"
            on:click={() => void switchResumeMode('master')}><FileText size={14} />主简历</button
          ><button
            class:active-mode={resumeMode === 'variant'}
            class="mode-button"
            on:click={() => void switchResumeMode('variant')}
            ><BriefcaseBusiness size={14} />岗位版本</button
          >
        </div>
        {#if resumeMode === 'variant' && variants.length}
          <select
            class="select h-9 w-[260px] py-1 text-xs"
            value={activeVariant?.id ?? ''}
            aria-label="选择岗位版本"
            on:change={(event) => void selectVariant(event.currentTarget.value)}
            ><option value="" disabled>选择已有岗位版本</option>{#each variants as variant}<option
                value={variant.id}
                >{variant.company} · {variant.jobTitle}{variant.stale ? ' · 待同步' : ''}</option
              >{/each}</select
          >
          <button class="btn h-9 text-xs" on:click={() => void startVariantCreation()}
            ><Plus size={14} />新建岗位版本</button
          >
        {/if}
      </div>
      <p class="eyebrow">{resumeMode === 'master' ? 'MASTER RESUME' : 'TAILORED RESUME'}</p>
      <h2 class="page-title mt-1">
        {resumeMode === 'master'
          ? '一份可信的主简历'
          : (activeVariant?.name ?? '为目标岗位创建定制版本')}
      </h2>
      <p class="mt-1 text-sm body-muted">
        {resumeMode === 'master'
          ? '结构化内容是唯一真源；手工修改和 AI 修改都会保存为可恢复的本地版本。'
          : '岗位版本固定主简历基线，专岗调整不会覆盖主简历。'}
      </p>
    </div>
    <div class="flex flex-wrap justify-end gap-2">
      ...
      />
      {#if resumeMode === 'master'}<button class="btn" disabled={importing} on:click={requestImport}
          ><Upload size={15} />{importing ? '正在解析…' : draft ? '重新导入' : '导入简历'}</button
        >{/if}
      {#if resumeMode === 'variant' && activeVariant?.stale}<button
          class="btn"
          disabled={rebaseLoading || !$snapshot.resume}
          title={$snapshot.resume ? '' : '主简历暂时不可用'}
          on:click={openRebase}
          ><RefreshCw size={15} class={rebaseLoading ? 'animate-spin' : ''} />同步主简历</button
        >{/if}
      <button class="btn" on:click={openVersionHistory}><History size={15} />版本历史</button>
      <button class="btn" disabled={!draft} on:click={() => (healthDrawerOpen = true)}
        ><ShieldCheck size={15} />体检 {draft ? healthReport.issues.length : 0}</button
      >
      <button
        class="btn"
        disabled={!draft || Boolean(resumeMode === 'variant' && !$snapshot.resume)}
        on:click={() => openAssistant(null)}><Sparkles size={15} />AI 对话</button
      >
      <button class="btn" disabled={!draft || rendering} on:click={renderPdf}
        ><Download size={15} />{rendering ? '正在渲染…' : '导出 PDF'}</button
      >
      {#if resumeMode === 'variant' && activeVariant}<button
          class="btn"
          on:click={deleteActiveVariant}
          aria-label="删除当前岗位版本"><Trash2 size={15} /></button
        >{/if}
      <button
        class="btn-primary"
        disabled={!draft ||
          saving ||
          !hasUnsavedChanges ||
          Boolean(resumeMode === 'master' && pendingExternalResume)}
        on:click={save}
        ><Save size={15} />{saving
          ? '正在保存…'
          : hasUnsavedChanges
  ...
  {#if pendingExternalResume}
    <div
      class="mb-4 flex shrink-0 items-center justify-between gap-4 rounded-xl border px-4 py-3"
      role="alert"
      style="border-color: var(--warning); background: var(--warning-soft);"
    >
      <div>
        <p class="text-sm font-semibold">检测到新的简历版本 {pendingExternalResume.version}</p>
        <p class="mt-1 text-xs body-muted">
          当前本地修改尚未覆盖；载入新版本会放弃这些未保存修改。
        </p>
      </div>
      <button
        class="btn shrink-0"
        on:click={() => pendingExternalResume && adoptResume(pendingExternalResume)}
        >载入最新版本</button
      >
    </div>
  {/if}
  ...
  {#if resumeMode === 'variant' && !draft}
    <div class="panel grid min-h-[500px] place-items-center p-8">
      <div class="w-full max-w-xl text-center">
        <span
          class="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-2xl bg-brand-soft text-brand"
          ><BriefcaseBusiness size={27} /></span
        >
        <h3 class="text-xl font-semibold">
          {variants.length ? '创建新的岗位定制简历' : '创建第一份岗位定制简历'}
        </h3>
        <p class="mt-2 text-sm leading-6 body-muted">
          从当前主简历即时复制；创建过程不会调用 AI，也不会改变主简历。
        </p>
        {#if !$snapshot.resume}<p class="mt-5 text-sm text-warning">
            请先在“主简历”模式导入或创建主简历。
          </p>{:else}<div class="mx-auto mt-6 flex max-w-lg gap-2">
            <input
              class="input min-w-0 flex-1"
              bind:value={jobQuery}
              on:input={() => void loadVariantJobs()}
              placeholder="搜索岗位或公司"
              aria-label="搜索岗位版本目标"
            /><select
              class="select min-w-0 flex-1"
              bind:value={selectedVariantJobId}
              aria-label="选择岗位版本目标"
              ><option value="">选择岗位</option>{#each jobOptions as option}<option
                  value={option.id}>{option.company} · {option.title}</option
                >{/each}</select
            ><button
              class="btn-primary shrink-0"
              disabled={!selectedVariantJobId || creatingVariant}
              on:click={createVariant}>{creatingVariant ? '创建中…' : '创建岗位版本'}</button
            >
          </div>{/if}
      </div>
    </div>
  {:else if !draft}
    <div class="panel min-h-[520px] p-8">
      <div class="mx-auto max-w-4xl text-center">
        <span
          class="mx-auto mb-4 grid h-16 w-16 place-items-center rounded-2xl bg-brand-soft text-brand"
          ><FileText size={27} /></span
        >
        <h3 class="text-xl font-semibold">导入现有简历，或从可信空白模板开始</h3>
        <p class="mt-2 text-sm leading-6 body-muted">
          模板只提供章节顺序与专业技能分组，不会预填或虚构个人经历。
        </p>
        <button class="btn-primary mt-5" on:click={requestImport}
          ><Upload size={16} />选择简历文件</button
        >
      </div>
      <div class="mx-auto mt-8 grid max-w-5xl grid-cols-4 gap-4">
        {#each RESUME_TEMPLATES as template}
          <article
            class="flex min-h-[170px] flex-col rounded-2xl border p-5 text-left transition hover:-translate-y-0.5"
            style="border-color: var(--line); background: var(--panel-soft);"
          >
            <p class="text-sm font-semibold">{template.label}</p>
            <p class="mt-2 flex-1 text-xs leading-5 body-muted">{template.description}</p>
            <div class="mt-4 flex flex-wrap items-center gap-3">
              {#if template.sample}<button
                  class="btn-ghost h-8 px-0 text-xs"
                  on:click={() => openTemplatePreview(template.id)}>查看完整示例</button
                >{/if}
              <button
                class="text-xs font-semibold text-brand"
                disabled={Boolean(templateCreating)}
                on:click={() => createFromTemplate(template.id)}
                >{templateCreating === template.id ? '正在创建…' : '使用此模板'}</button
              >
            </div>
          </article>
        {/each}
  ...
  {:else}
    <div class="grid min-h-0 flex-1 grid-cols-[minmax(460px,1fr)_minmax(380px,.82fr)] gap-5">
      <section class="panel flex min-h-0 flex-col overflow-hidden">
        <nav class="flex shrink-0 gap-6 border-b px-6 pt-4" style="border-color: var(--line);">
          <button
            ...
            class="tab"
            on:click={() => (activeSection = 'content')}>简历内容</button
          >
          {#if resumeMode === 'master'}
            <button
              class:active={activeSection === 'preferences'}
              class="tab"
              on:click={() => (activeSection = 'preferences')}>求职偏好</button
            >
            <button
              class:active={activeSection === 'facts'}
              class="tab"
              on:click={() => (activeSection = 'facts')}
              >事实清单 <span class="ml-1 text-brand">{draft.facts.length}</span></button
            >
          {:else}
            <button
              class:active={activeSection === 'coverage'}
              class="tab"
              on:click={() => (activeSection = 'coverage')}>岗位覆盖</button
            >
            <button
              class:active={activeSection === 'preferences'}
              class="tab"
              on:click={() => (activeSection = 'preferences')}>求职偏好 · 只读</button
            >
            <button
              class:active={activeSection === 'facts'}
              class="tab"
              on:click={() => (activeSection = 'facts')}
              >事实清单 · 只读 <span class="ml-1 text-brand">{draft.facts.length}</span></button
            >
          {/if}
        </nav>
              ...
              <div class="animate-lift space-y-4">
                ...
                >
                  <div class="flex gap-3">
                    <ShieldCheck size={18} class="mt-0.5 shrink-0 text-brand" />
                    <div>
                      <p class="text-sm font-semibold">来自主简历基线的只读偏好</p>
                      <p class="mt-1 text-xs leading-5 body-muted">
                        岗位版本不维护独立偏好；同步主简历后会刷新这里的快照。
                      </p>
                    </div>
                  </div>
                </div>
                <dl class="grid grid-cols-2 gap-3 text-sm">
                  <div class="readonly-card">
                    <dt>目标岗位</dt>
                    <dd>{draft.preferences.targetRoles.join('、') || '未填写'}</dd>
                  </div>
                  <div class="readonly-card">
                    <dt>目标城市</dt>
                    <dd>{draft.preferences.cities.join('、') || '未填写'}</dd>
                  </div>
                  <div class="readonly-card">
                    <dt>工作方式</dt>
                    <dd>{draft.preferences.remotePreference || '未填写'}</dd>
                  </div>
                  <div class="readonly-card">
                    <dt>让你有动力的任务</dt>
                    <dd>{draft.preferences.energizingTasks.join('、') || '未填写'}</dd>
                  </div>
                  <div class="readonly-card">
                    <dt>希望减少的任务</dt>
                    <dd>{draft.preferences.drainingTasks.join('、') || '未填写'}</dd>
                  </div>
                  <div class="readonly-card">
                    <dt>不可妥协的条件</dt>
                    <dd>{draft.preferences.hardConstraints.join('、') || '未填写'}</dd>
                  </div>
                </dl>
              </div>
            {:else}
              <div class="animate-lift">
                ...
                >
                  <div class="flex gap-3">
                    ...
                    <div>
                      <p class="text-sm font-semibold">约 60 秒，提高匹配结果的可信度</p>
                      <p class="mt-1 text-xs leading-5 body-muted">
                        不知道的维度不会被 AI 硬猜。未填写项会降低置信度，而不会直接扣分。
                      </p>
                    </div>
                  </div>
                </div>
                <div class="mt-6 space-y-5">
                  <label
                    ><span class="label">目标岗位</span><input
                      class="input"
                      bind:value={targetRolesText}
                      placeholder="AI Agent、大模型应用"
                    /><span class="mt-1 block text-[11px] body-muted">使用顿号分隔多个方向</span
                    ></label
                  ><label
                    ><span class="label">目标城市</span><input
                      class="input"
                      bind:value={citiesText}
                      placeholder="上海、杭州"
                    /></label
                  ><label
                    ><span class="label">工作方式</span><select
                      class="select"
                      bind:value={draft.preferences.remotePreference}
                      ><option value="onsite">到岗办公</option><option value="hybrid"
                        >混合办公</option
                      ><option value="remote">远程办公</option><option value="flexible">均可</option
                      ></select
                    ></label
                  ><label
                    ><span class="label">让你有动力的任务</span><textarea
                      class="textarea"
                      bind:value={energizingText}
                      placeholder="从 0 到 1、系统设计、Agent 工作流"></textarea></label
                  ><label
                    ><span class="label">希望减少的任务</span><textarea
                      class="textarea"
                      bind:value={drainingText}
                      placeholder="长期纯维护、高频汇报"></textarea></label
                  ><label
                    ><span class="label">不可妥协的条件</span><textarea
                      class="textarea"
                      bind:value={constraintsText}
                      placeholder="不接受长期出差"></textarea></label
                  >
                  <div class="flex justify-end">
                    <button class="btn-primary" on:click={savePrefs}
                      ><Check size={15} />保存求职偏好</button
                    >
                  </div>
                </div>
              </div>
              ...
              <div class="animate-lift space-y-4">
                ...
                >
                  <div class="flex gap-3">
                    <ShieldCheck size={18} class="mt-0.5 shrink-0 text-brand" />
                    <div>
                      <p class="text-sm font-semibold">来自主简历基线的只读事实</p>
                      <p class="mt-1 text-xs leading-5 body-muted">
                        新增事实需要先回到主简历确认，再手动同步到岗位版本。
                      </p>
                    </div>
                  </div>
                </div>
                {#if draft.facts.length}
                  <div class="space-y-2">
                    {#each draft.facts as fact}<article class="readonly-card">
                        <div class="flex items-center justify-between gap-3">
                          <span class="text-xs font-semibold text-brand">{fact.category}</span><span
                            class="text-[11px] body-muted"
                            >{fact.confirmed ? '已确认' : '未确认'}</span
                          >
                        </div>
                        <p class="mt-2 text-sm leading-6">{fact.value}</p>
                        <p class="mt-1 text-[11px] body-muted">{fact.source}</p>
                      </article>{/each}
                  </div>
                {:else}
                  <p
                    class="rounded-xl border p-5 text-sm body-muted"
                    style="border-color: var(--line);"
                  >
                    基线主简历还没有事实记录。
                  </p>
      ...
      <section class="panel flex min-h-0 flex-col overflow-hidden">
        <div
          class="flex h-[53px] shrink-0 items-center justify-between border-b px-5"
          style="border-color: var(--line);"
        >
          <div class="flex items-center gap-2">
            <FileText size={16} class="text-brand" /><span class="text-sm font-semibold"
              >RenderCV 预览</span
            >
          </div>
          <div class="flex items-center gap-3 text-[11px] body-muted">
            <span>{hasUnsavedChanges ? '有未保存修改' : '内容已保存'}</span><button
              class="btn-ghost h-8"
              on:click={openVersionHistory}><History size={14} />版本 {draft.version}</button
            >
          </div>
        </div>
        <div
          class="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-6"
          style="background: var(--panel-soft);"
        >
          <ResumePaper
            resume={draft}
            sections={previewSections}
            coverageKeywords={previewCoverageKeywords}
          />
        </div>
      </section>
    </div>
...
<ResumeChatDialog
  ...
  target={draft
    ? {
        kind: resumeMode,
        id: resumeMode === 'variant' ? (activeVariant?.id ?? draft.id) : draft.id
      }
    : null}
...
{#if previewTemplate?.sample}
  <button
    class="fixed inset-0 z-[70] bg-black/35 backdrop-blur-sm"
    tabindex="-1"
    on:click={() => (previewTemplate = null)}
    aria-label="关闭模板示例"
  ></button>
  <div
    class="fixed inset-y-5 left-1/2 z-[71] flex w-[760px] max-w-[calc(100vw-32px)] -translate-x-1/2 flex-col overflow-hidden panel"
    role="dialog"
    aria-modal="true"
    aria-labelledby="template-preview-title"
    tabindex="-1"
    use:modalFocus={{ close: () => (previewTemplate = null) }}
  >
    <div
      class="flex shrink-0 items-start justify-between gap-4 border-b px-6 py-5"
      style="border-color: var(--line);"
    >
      <div>
        <p class="eyebrow">完整示例</p>
        <h3 id="template-preview-title" class="mt-1 text-xl font-semibold">
          {previewTemplate.label}简历
        </h3>
        <p class="mt-1 text-xs body-muted">3–5 年社招候选人写法参考</p>
      </div>
      <button class="btn-icon" on:click={() => (previewTemplate = null)} aria-label="关闭">×</button
      >
    </div>
    <div
      class="mx-6 mt-5 shrink-0 rounded-xl border px-4 py-3 text-sm font-semibold text-warning"
      style="border-color: color-mix(in srgb, var(--warning) 35%, var(--line)); background: var(--warning-soft);"
    >
      示例内容，请勿直接用于投递。示例不会写入主简历或事实库。
    </div>
    <div
      class="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-6"
      style="background: var(--panel-soft);"
    ...
    <div
      ...
      style="border-color: var(--line);"
    >
      <p class="text-xs body-muted">
        {draft ? '只切换章节结构，保留当前全部内容。' : '创建时只使用空白结构和技能分组。'}
      </p>
      <button class="btn-primary shrink-0" on:click={usePreviewedTemplate}
        >{draft ? '切换至此结构' : '使用此模板'}</button
      >
    </div>
  </div>
{/if}
...
{#if toast}<div
    class="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-[#1d2824] px-4 py-2.5 text-sm font-medium text-white shadow-xl animate-lift"
  >
    {toast}
  </div>{/if}
...
<style>
  ...
  .export-theme-card.selected-export {
    border-color: var(--swatch-accent);
    transform: translateY(-2px);
  }
  .export-theme-card.selected-export {
    box-shadow: 0 0 0 2px color-mix(in srgb, var(--swatch-accent) 24%, transparent);
  }
  .export-swatch {
    display: flex;
    height: 104px;
  ...
  .export-swatch span:first-child {
    width: 58%;
    height: 9px;
    background: var(--swatch-accent);
  }
  .export-swatch span:nth-child(2) {
    background: var(--swatch-accent);
    opacity: 0.72;
  }
</style>
