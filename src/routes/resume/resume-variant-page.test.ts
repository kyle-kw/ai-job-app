...
describe('resume variant page', () => {
  it('creates one job-bound copy without AI and saves it independently', async () => {
    ...
    const detail: ResumeVariantDetail = {
      id: 'variant-1', jobId: mockJobs[0].id, jobTitle: mockJobs[0].title, company: mockJobs[0].company,
      name: `${mockJobs[0].company} · ${mockJobs[0].title}`, baseResumeId: master.id, baseResumeVersion: master.version,
      version: 1, createdAt: now, updatedAt: now, stale: false,
      profile: { ...structuredClone(master), id: 'variant-1', version: 1, updatedAt: now }
    };
    const { profile: _profile, ...summary } = detail;
    let created = false;
    vi.spyOn(backend, 'listResumeVariants').mockImplementation(async () => created ? [summary] : []);
    vi.spyOn(backend, 'listJobOptions').mockResolvedValue([{ id: mockJobs[0].id, title: mockJobs[0].title, company: mockJobs[0].company, lastSeen: mockJobs[0].lastSeen }]);
    vi.spyOn(backend, 'getResumeVariant').mockResolvedValue(detail);
    vi.spyOn(backend, 'getJob').mockResolvedValue(mockJobs[0]);
    const create = vi.spyOn(backend, 'createResumeVariant').mockImplementation(async () => { created = true; return detail; });
    ...
    const save = vi.spyOn(backend, 'saveResumeVariant').mockImplementation(async (_id, resume) => ({
      ...
      version: { id: 'version-2', resumeId: detail.id, version: 2, parentVersion: 1, createdAt: now, source: 'variant-manual', summary: '手工保存岗位版本' }
    }));
    const renderPdf = vi.spyOn(backend, 'renderResume').mockResolvedValue({ path: 'variant.pdf', fileName: 'variant.pdf' });
    ...
    await fireEvent.change(screen.getByLabelText('选择岗位版本目标'), { target: { value: mockJobs[0].id } });
    ...
    await fireEvent.input(screen.getByLabelText('职业标题'), { target: { value: '专岗 AI 平台工程师' } });
    await fireEvent.click(screen.getByRole('button', { name: '保存修改' }));
    await waitFor(() => expect(save).toHaveBeenCalledWith(detail.id, expect.objectContaining({ headline: '专岗 AI 平台工程师' }), 1));
    ...
    await waitFor(() => expect(renderPdf).toHaveBeenCalledWith(expect.objectContaining({
