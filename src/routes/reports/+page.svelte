<script lang="ts">
  ...
  type KeywordReportBackend = {
    ...
    generateReportCompetitiveness: (keywordKeys: string[], force?: boolean) => Promise<ReportCompetitivenessState>;
  };
  ...
  async function loadKeywords() {
    ...
    try {
      ...
      const requested = currentUrl?.pathname === '/reports' ? currentUrl.searchParams.getAll('keyword') : [];
      const validRequested = keywords.filter((keyword) => requested.includes(keyword.key)).map((keyword) => keyword.key);
  ...
  async function loadReportCompetitiveness() {
    ...
    try {
      ...
      if (requestId === competitivenessRequestId) competitivenessError = reason instanceof Error ? reason.message : String(reason);
    } finally {
  ...
  const skillPairHref = (row: ReportBucket) => drilldownHref({ skills: row.label.split(' × ').map((item) => item.trim()).filter(Boolean) });
  ...
  const salaryBandCode = (label: string): ReportSalaryBand => ({
    '15K 以下': 'under-15', '15–25K': '15-25', '25–35K': '25-35', '35–50K': '35-50', '50K 以上': '50-plus'
...
<div class="page-content report-page">
  <header class="mb-6 flex items-start justify-between gap-5">
    ...
    <div class="flex shrink-0 gap-2">
      <button class="btn" on:click={() => { void loadReport(); void loadInterviewPreparation(); void loadReportCompetitiveness(); }} disabled={loading || interviewGenerating || competitivenessGenerating || selectedKeywordKeys.length === 0}><RefreshCw size={15} class={loading ? 'animate-spin' : ''} />刷新当前范围</button>
  ...
  {:else}
    ...
    <section class="panel mt-5 overflow-hidden" aria-labelledby="competitiveness-title">
      ...
      {:else if !competitivenessState}
        <div class="p-6"><div class="rounded-xl border p-4 text-sm" style="border-color: var(--line);"><p class="font-semibold">暂时无法读取竞争力分析</p><p class="mt-1 text-xs body-muted">{competitivenessError || '请稍后重试。'}</p><button class="btn mt-3" on:click={loadReportCompetitiveness}>重试</button></div></div>
      {:else if !competitivenessState.hasResume}
        <div class="flex flex-wrap items-center justify-between gap-4 p-6"><div><p class="text-sm font-semibold">先建立可信主简历</p><p class="mt-1 text-xs leading-5 body-muted">导入或创建主简历后，才能判断已覆盖、可强化和真实缺口。</p></div><a href="/resume" class="btn-primary">前往简历</a></div>
      {:else}
        <div class="p-6">
          {#if !competitivenessState.hasProvider}<div class="mb-5 flex flex-wrap items-center justify-between gap-4 rounded-xl border p-4" style="border-color: var(--line); background: var(--panel-soft);"><div><p class="text-sm font-semibold">当前展示本地精确匹配</p><p class="mt-1 text-xs body-muted">配置并验证模型后，可以主动运行一次语义复核。</p></div><a href="/settings" class="btn">配置语义分析模型</a></div>{/if}
    ...
    <section class="mt-7">
      ...
      <div class="grid grid-cols-2 gap-5"><article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">岗位需要哪些技能</h4><ReportBars rows={report.topSkills} limit={14} hrefForRow={skillHref} /></article><article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">技能共现组合</h4><ReportBars rows={report.skillPairs} limit={10} hrefForRow={skillPairHref} /></article></div>
    </section>

    <section class="mt-7">
      <div class="mb-3"><p class="eyebrow">Candidate requirements</p><h3 class="section-title mt-1">薪资与候选人门槛</h3></div>
      <div class="grid grid-cols-3 gap-5">
        <article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">薪资区间分布</h4><ReportBars rows={report.salary.bands} hrefForRow={salaryBandHref} /><div class="mt-5 grid grid-cols-3 gap-2 border-t pt-4 text-center" style="border-color: var(--line);"><div><strong class="block text-sm">{salary(report.salary.medianLowK)}</strong><span class="text-[11px] body-muted">下限中位数</span></div><div><strong class="block text-sm">{salary(report.salary.medianHighK)}</strong><span class="text-[11px] body-muted">上限中位数</span></div><div><strong class="block text-sm">{report.salary.extraMonthsCount}</strong><span class="text-[11px] body-muted">13 薪及以上</span></div></div></article>
        <article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">经验要求</h4><ReportBars rows={report.experience} hrefForRow={experienceHref} /></article>
        <article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">学历要求</h4><ReportBars rows={report.degree} /></article>
      </div>
      {#if report.salaryByExperience.length > 0}<article class="panel mt-5 p-6"><h4 class="mb-5 text-sm font-semibold">不同经验要求的薪资中位数</h4><div class="grid grid-cols-5 gap-3">{#each report.salaryByExperience.slice(0, 5) as item}<a href={drilldownHref({ experience: item.label })} class="rounded-xl p-4 surface-soft transition hover:ring-2 hover:ring-[var(--brand)]"><span class="text-xs body-muted">{item.label}</span><strong class="mt-2 block text-xl tabular-nums">{item.medianK.toFixed(1)}K</strong><span class="text-[11px] body-muted">{item.count} 个薪资样本</span></a>{/each}</div></article>{/if}
    </section>

    <section class="mt-7">
      <div class="mb-3"><p class="eyebrow">Market structure</p><h3 class="section-title mt-1">市场结构</h3></div>
      <div class="grid grid-cols-2 gap-5"><article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">岗位方向</h4><ReportBars rows={report.roles} /></article><article class="panel p-6"><h4 class="mb-5 flex items-center gap-2 text-sm font-semibold"><MapPinned size={15} class="text-brand" />城市分布</h4><ReportBars rows={report.cities} hrefForRow={cityHref} /></article><article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">行业分布</h4><ReportBars rows={report.industries} /></article><article class="panel p-6"><h4 class="mb-5 text-sm font-semibold">公司规模</h4><ReportBars rows={report.companyScales} /></article><article class="panel col-span-2 p-6"><h4 class="mb-5 text-sm font-semibold">常见福利</h4><div class="max-w-4xl"><ReportBars rows={report.welfare} limit={12} /></div></article></div>
    </section>
...
<style>
  ...
  .item-rise { color: var(--success); }
  .item-fall { color: var(--danger); }
  .item-covered { background: color-mix(in srgb, #2f855a 12%, var(--panel)); color: #2f855a; }
  .item-strengthenable { background: color-mix(in srgb, #3182ce 12%, var(--panel)); color: #2467a6; }
  .item-gap { background: var(--danger-soft); color: var(--danger); }
